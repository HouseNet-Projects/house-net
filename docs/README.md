...
- Governance and enforcement: `house-net-control.json`, `control-plane/policy/`, `control-plane/policy/enforcement-matrix.json`, `tools/validate_house_net.py`
...
Critical rules are mapped to executable validators, runtime gates, tests and CI in `control-plane/policy/enforcement-matrix.json`. Run `python tools/validate_house_net.py` and `control-plane/bin/housenet-preflight --json` before reviewing a change.

Կրիտիկական կանոնները executable validator-ների, runtime gate-երի, թեստերի և CI-ի հետ կապված են `control-plane/policy/enforcement-matrix.json`-ում։ Փոփոխություն վերանայելիս գործարկիր `python tools/validate_house_net.py` և `control-plane/bin/housenet-preflight --json`։
